...
let questionArray = [
  ...
  {
    question:
      'In the JavaScript array "spam = [1, 2, 3, 4, 5]", what is spam[1] equal to?',
  ...
  {
    question:
      'If you were a detective looking for the quiz answers, where might you look?',
    answers: [
      "Inspect the DOM for the button with id='correct'",
...

  {
    ...
    answers: [
      'Are People Insane?!',
      'Asynchronous Possibility Interweaving',
      'Always Program Intelligently',
...
function initialQuiz() {
  ...
  quizLayout.innerHTML =
    'Answer all the questions within the time limit! Incorrect answers will subtract ' +
    PENALTYTIME +
    ' seconds from the time.'
  startBtn.style.display = 'visible'

  // retrieve high scores lists from local storage.
  let storedHighScoresArray = JSON.parse(
    localStorage.getItem('highestScoresArray')
  )
  if (storedHighScoresArray !== null) {
    highestScores = storedHighScoresArray
  }
  let storedHighScoreList = JSON.parse(
...
document
  .getElementById('top-left-high-scores')
...
if (INITIALSONLY) {
  document
    .getElementById('name-text')
    .setAttribute('placeholder', 'Your initials here')
}
...
function inputName(highScore) {
  ...
  if (INITIALSONLY) {
    ...
    alert(
      'Maximum character length for names is ' + MAXNAMELENGTH + ' characters!'
    )
    return false
  }
